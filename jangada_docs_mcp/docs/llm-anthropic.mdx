...
- **Structured output** (`parse`): por **tool-forcing** — define uma ferramenta
  ...
  que **não aceitam forced tool use** (Opus 5.5, Fable 5.1 e Mythos 5.1), o `parse` usa
  `tool_choice="auto"` + **strict tool use** automaticamente — transparente para
  você. Veja "O que mudou na 1.9.2".
...
- **Compatível com Claude Opus 5.5, Fable 5.1 e Mythos 5.1** (lançados em 2026-09-22). Esses
  modelos trazem um breaking change: **forced tool use não é mais aceito** —
  `tool_choice` do tipo `"tool"`/`"any"` retorna 400. Como o structured output
  usava tool-forcing, o `parse` quebrava nesses modelos. Agora, só neles, o `parse`
  usa `tool_choice="auto"` + strict tool use (a migração recomendada pela Anthropic);
  os demais modelos Claude seguem no tool-forcing de sempre. É transparente: você
  chama `LLM("anthropic", "claude-opus-5-5").parse(...)` normalmente.
- **`complete(tools=…, tool_choice="required"` (ou `"any"`/nome) no Opus 5.5 / Fable
  5.1 / Mythos 5.1** agora levanta `UnsupportedError` claro (a API rejeitaria com 400). Use
  `tool_choice="auto"` — se quiser garantir o schema dos argumentos, a lib já aplica
  strict tool use no caminho de `parse`.
