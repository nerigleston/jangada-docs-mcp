...
<Callout type="warn">
  **`max_tokens` tem default de `1024`.** Se você não passar `max_tokens`, toda
  resposta é cortada nesse limite — silenciosamente, sem erro. O sintoma mais
  comum é **structured output de listas grandes que volta truncado** (JSON
  incompleto / Pydantic falhando ao validar). Para extrações longas, suba o
  limite explicitamente:

  ```python
  llm = LLM("openai", "gpt-5", max_tokens=8192)
  # ou só nesta chamada:
  llm.parse("Extraia todos os itens.", Itens, files=[...], params={"max_tokens": 8192})
  ```
</Callout>
